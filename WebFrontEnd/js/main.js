...
var pad;
var MQTTclient = mqtt.connect("ws://192.168.43.86:1337", {
    clientId: "RBWEB-" + Math.floor(999 * Math.random())
})
var DHTmsg = "", Presencemsg = ""
...
function start() {
    ...
    setInterval(function(){
        MQTTclient.publish("sensor", "read_temp")
        MQTTclient.publish("sensor", "read_presence")
    },2000)
    MQTTclient.on("message", function (topic, payload) {
            ...
            if (topic.localeCompare('sensor_data') == 0) { 
                if(payload.indexOf("%") != -1){ // DHT
                    console.log(payload + "!")
                    DHTmsg = payload
                }else if(payload.indexOf("_here") != -1){ // Presence
